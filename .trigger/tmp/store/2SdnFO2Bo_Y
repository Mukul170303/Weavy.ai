import {
  ...
  cropImageTask,
  extractFrameTask
} from "../../../chunk-RLC6WJLK.mjs";
import {
  task
} from "../../../chunk-2OMWR522.mjs";
import "../../../chunk-GUCSW7BX.mjs";
import {
  __name,
  init_esm
...
function getLayeredExecutionPlan(nodes, edges) {
...
__name(getLayeredExecutionPlan, "getLayeredExecutionPlan");
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      ...
      if ((run.executionScope === "SELECTED" || run.executionScope === "SINGLE") && run.selectedNodeIds) {
        const targetIds = JSON.parse(run.selectedNodeIds);
        console.log(`[Orchestrator] Filtering for ${targetIds.length} target nodes: ${targetIds.join(", ")}`);
        layeredPlan = layeredPlan.map((layer) => layer.filter((node) => targetIds.includes(node.id))).filter((layer) => layer.length > 0);
      }
    } catch (err) {
      await prisma.workflowRun.update({
        where: { id: run.id },
        data: { status: "FAILED", finishedAt: /* @__PURE__ */ new Date() }
      });
      throw err;
    }
    const context = {};
    ...
    await prisma.workflowRun.update({
      where: { id: run.id },
      data: { status: "COMPLETED", finishedAt: /* @__PURE__ */ new Date() }
    });
    return { success: true };
  }, "run")
});
async function executeLLMNode(node, edges, context, runId) {
...
` : "") + sourceData.text;
  ...
  try {
    ...
    if (result.ok) {
      ...
      const duration = Math.round((finishedAt.getTime() - executionRecord.startedAt.getTime()) / 1e3);
...
async function executeExtractFrameNode(node, edges, context, runId) {
...
__name(executeExtractFrameNode, "executeExtractFrameNode");
async function executeCropImageNode(node, edges, context, runId) {
  ...
  const paramHandles = ["x", "y", "width", "height"];
...
__name(executeCropImageNode, "executeCropImageNode");
async function handleNodeFailure(executionRecordId, error) {
  await prisma.nodeExecution.update({
...
__name(handleNodeFailure, "handleNodeFailure");
export {
  orchestrator
};

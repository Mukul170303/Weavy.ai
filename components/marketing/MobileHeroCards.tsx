...
export default function MobileHeroCards() {
  return (
    <div className="relative px-4 pb-8 text-black">
      ...
      <div className="relative rounded-tl-[30px] overflow-hidden pt-6 pb-8 bg-gradient-to-b from-[#DDE1E3] to-[#F6F7F8]">
        ...
        <div className="relative space-y-4 px-4">
          ...
          {/* Connection line */}
          <svg
            className="absolute top-[15%] left-[40%] w-20 h-24 overflow-visible"
            aria-hidden="true"
          >
            <path
...
function CardItem({ card }: CardProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5 text-black">
      ...
      {/* Label */}
      <div className="flex items-center gap-2 text-[9px] font-medium tracking-[0.12em] uppercase text-black/70">
        <span>{card.type}</span>
        {card.label && (
      ...
      {/* Content */}
      <div
        className={`${heightClass} w-full rounded-lg overflow-hidden bg-white/40 relative`}
      >
        {card.image ? (
          <img
            src={card.image}
        ...
        {/* Connection dots */}
        <div className="absolute top-1/2 -right-1.5 w-2.5 h-2.5 rounded-full bg-black/30 transform -translate-y-1/2" />
        <div className="absolute top-1/2 -left-1.5 w-2.5 h-2.5 rounded-full bg-black/30 transform -translate-y-1/2" />
      </div>
